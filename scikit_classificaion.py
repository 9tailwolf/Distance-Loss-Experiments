...
def main(args=None):
    ...
    model = NN(inp=99,linear=250,dropout=0.3,labels=args.label).to(device=device) # You can edit linear(intenger), dropout(float f, 0<= f <=1).
    trainer = Trainer(train_data=train_dataloader, test_data=test_dataloader, model=model,lr=5e-5,eps=1e-8,epochs=50, loss=args.loss) # You can edit lr(learning rate), eps(epsilons that prevent gradient vanishing problem), and epochs(intenger)
    trainer.train()
